...
tic
T=zeros(No_TrainingSample,7);%T is training instances where dim1,dim2, dim3,dim4,dim5, dim6 and dim7 are cost, parent, label, root, bc, weight and ring index, respectively
T(:,1)=100000;
Z1=P_Set(:,1:8);
...
for alpha=1:alpha_No
    ...
    for j=1:radiusSplitNo        
        ...
        bounds(3,:)=bounds(3,:)+0.0001;% modify pi edges of angles
        bounds(5,:)=bounds(5,:)+0.0001;% modify 2*pi edges of angles for theta angle
        result = 1:4;
        b = 1:2;
        for i=1:m-2
        ...
        for p=1:size(PrototypesIndex,1)
            ...
            radiusItr(j+1)=max(p_i(:,8))+0.1;% Beacuse some rings from other rings can be belonged to the current OPT
